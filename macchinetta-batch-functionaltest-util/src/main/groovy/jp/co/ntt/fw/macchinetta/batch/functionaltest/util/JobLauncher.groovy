 ...
 *  <ol>
 *    <li> Please set to java working directory to "/macchinetta-functionaltest/macchinetta-batch-functionaltest"</li>
 *    <li> Change directory to /macchinetta-functionaltest, Execute "mvn clean package" in advance.</li>
 *  </ol>
...
class JobLauncher {
    ...
    int syncJob(JobRequest jobRequest, long timeout = 600 * 1000L, TimeUnit timeUnit = TimeUnit.MILLISECONDS,
        ...
        def command = "java -cp target/dependency/* ${systemProperties} ${remoteDebug} ${CommandLineJobRunner.class.name} " +
    ...
    synchronized void stopAsyncBatchDaemon(Process p, long timeout = 120 * 1000L, TimeUnit timeUnit = TimeUnit.MILLISECONDS) {
        File f = stopFile()
