...
/**
 * JavaConfig class for JobReadFixedLengthSeparateFixedLengthWithLinebreaks job.
 *
 * @since 2.5.0
 */
@Configuration
...
public class JobReadFixedLengthSeparateFixedLengthWithLinebreaksConfig {

    @Bean
    @StepScope
    public FlatFileItemReader<SalesPlanDetail> reader(
            @Value("#{jobParameters['inputFile']}") File inputFile,
            @Value("#{jobParameters['readCharsetName']}") String readCharsetName,
            @Value("#{jobParameters['tokenizeCharsetName']}") String tokenizeCharsetName) {
        Range[] ranges = new Range[] {new Range(1, 6), new Range(7, 10), new Range(11, 12), new Range(13, 22), new Range(23, 32)};
        final FixedByteLengthLineTokenizer tokenizer = new FixedByteLengthLineTokenizer(ranges, Charset.forName(tokenizeCharsetName));
    ...
    public Step step01(JobRepository jobRepository,
        ...
        return new StepBuilder("jobReadFixedLengthSeparateFixedLengthWithLinebreaks.step01",
                jobRepository)
    ...
    @Bean
    public Job jobReadFixedLengthSeparateFixedLengthWithLinebreaks(JobRepository jobRepository,
                                            Step step01,
                                            JobExecutionLoggingListener listener) {
        return new JobBuilder("jobReadFixedLengthSeparateFixedLengthWithLinebreaks",jobRepository)
                .start(step01)
                .listener(listener)
                .build();
    }
