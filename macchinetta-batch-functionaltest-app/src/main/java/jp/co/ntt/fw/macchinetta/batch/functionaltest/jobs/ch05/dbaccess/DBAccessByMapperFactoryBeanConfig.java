...
import jp.co.ntt.fw.macchinetta.batch.functionaltest.app.repository.performance.SalesPerformanceSummaryRepository;
import jp.co.ntt.fw.macchinetta.batch.functionaltest.config.JobBaseContextConfig;

/**
 * JavaConfig class for DBAccessByMapperFactoryBean job.
 *
 * @since 2.5.0
 */
@Configuration
...
public class DBAccessByMapperFactoryBeanConfig {
    
    @Bean
    public MapperFactoryBean<SalesPerformanceDetailRepository> salesPerformanceDetailRepository(
            @Qualifier("batchModeSqlSessionTemplate") SqlSessionTemplate sqlSessionTemplate) {
        final MapperFactoryBean<SalesPerformanceDetailRepository> mapperFactoryBean = new MapperFactoryBean<>();
        mapperFactoryBean.setMapperInterface(
                SalesPerformanceDetailRepository.class);
        mapperFactoryBean.setSqlSessionTemplate(sqlSessionTemplate);
        return mapperFactoryBean;
    }
    
    @Bean
    public MapperFactoryBean<SalesPerformanceSummaryRepository> salesPerformanceSummaryRepository(
            @Qualifier("batchModeSqlSessionTemplate") SqlSessionTemplate sqlSessionTemplate) {
        final MapperFactoryBean<SalesPerformanceSummaryRepository> mapperFactoryBean = new MapperFactoryBean<>();
        mapperFactoryBean.setMapperInterface(
                SalesPerformanceSummaryRepository.class);
        mapperFactoryBean.setSqlSessionTemplate(sqlSessionTemplate);
        return mapperFactoryBean;
    }
    ...
    public Step step01(JobRepository jobRepository,
        ...
        return new StepBuilder("DBAccessByMapperFactoryBean.step01",
                jobRepository)
    ...
    @Bean
    public Job DBAccessByMapperFactoryBean(JobRepository jobRepository,
                                            Step step01,
                                            JobExecutionLoggingListener listener) {
        return new JobBuilder("DBAccessByMapperFactoryBean",
                jobRepository)
                .start(step01)
                .listener(listener)
                .build();
