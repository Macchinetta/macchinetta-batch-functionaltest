...
/**
 * JavaConfig class for JobInvoice02 job.
 *
 * @since 2.5.0
 */
@Configuration
...
@ComponentScan({ "jp.co.ntt.fw.macchinetta.batch.functionaltest.app.common",
        "jp.co.ntt.fw.macchinetta.batch.functionaltest.app.plan" })
...
public class JobInvoice02Config {

    @Bean
    @StepScope
    public MyBatisCursorItemReader<SalesPerformanceDetail> summarizeInvoice(
    ...
    public FlatFileItemWriter<SalesPerformanceDetail> summaryWriter(
        ...
        final BeanWrapperFieldExtractor<SalesPerformanceDetail> fieldExtractor = new BeanWrapperFieldExtractor<>();
        fieldExtractor.setNames(
                new String[] { "branchId", "year", "month",
                        "customerId", "amount" });
        final DelimitedLineAggregator<SalesPerformanceDetail> lineAggregator = new DelimitedLineAggregator<>();
    ...
    public Step step01(JobRepository jobRepository,
                       ...
                       MyBatisCursorItemReader<SalesPerformanceDetail> summarizeInvoice,
                       FlatFileItemWriter<SalesPerformanceDetail> summaryWriter) {
        return new StepBuilder("jobInvoice02.step01", jobRepository)
                .<SalesPerformanceDetail, SalesPerformanceDetail> chunk(10, transactionManager)
                .reader(summarizeInvoice)
                .writer(summaryWriter)
                .build();
    }

    @Bean
    public Job jobInvoice02(JobRepository jobRepository, Step step01,
                                 @Qualifier("jobExecutionLoggingListener") JobExecutionLoggingListener listener) {
        return new JobBuilder("jobInvoice02", jobRepository)
                .start(step01)
                .listener(listener)
                .build();
    }
