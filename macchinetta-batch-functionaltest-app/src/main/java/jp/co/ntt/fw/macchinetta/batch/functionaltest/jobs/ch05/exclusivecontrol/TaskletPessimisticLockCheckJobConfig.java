...
import jp.co.ntt.fw.macchinetta.batch.functionaltest.ch05.exclusivecontrol.PessimisticLockTasklet;
import jp.co.ntt.fw.macchinetta.batch.functionaltest.config.JobBaseContextConfig;

/**
 * JavaConfig class for TaskletPessimisticLockCheckJob.
 *
 * @since 2.5.0
 */
@Configuration
...
public class TaskletPessimisticLockCheckJobConfig {

    @Bean
    public Step step01(JobRepository jobRepository,
                       @Qualifier("jobTransactionManager") PlatformTransactionManager transactionManager,
                       PessimisticLockTasklet pessimisticLockTasklet) {
        return new StepBuilder("taskletPessimisticLockCheckJob.step01",
                jobRepository)
                .tasklet(pessimisticLockTasklet, transactionManager)
                .build();
    }
    
    @Bean
    public Job taskletPessimisticLockCheckJob(JobRepository jobRepository,
                                            Step step01,
                                            JobExecutionLoggingListener listener) {
        return new JobBuilder("taskletPessimisticLockCheckJob",jobRepository)
                .start(step01)
                .listener(listener)
                .build();
    }
